...
The code below calculates and visualizes profit based on yield results from Assignment 2 using default parameters.
...
The following simple informal sensitivity analysis assumes uncertainty in *temp_param2* and *precip_param2* parameters, with default values from the function -0.0046 and 0.0043, respectively.
...
The parameter combinations used for the sensitivity analysis are plotted below.
...
Using the results from the previous section, we can plot yearly profit distribution given parameter uncertainty.

```{r}
profit_sensitivity <- profit_model(yield_results = mean_yield)
```

First, the relationship between profit and almond yield is visualized. Our profit model estimates that as yield increases, profit (in terms of $/acre) will gradually decrease. Theoretically, this is potentially caused by decreased price per unit (i.e. law of demand/supply) and increasing marginal costs of production.
...
Lastly, we plot yearly profit distribution given parameter uncertainty. In our submitted graph, 1995 is separated due to the large difference in scale.

```{r}
p1 <- profit_sensitivity %>% filter(year != 1995) %>% ggplot(aes(x=factor(year), y=profit, group=year)) +
  geom_boxplot() +
  theme_light() +
  theme(axis.text.x = element_text(angle=90)) +
  labs(x="Year", y="Profit ($/acre)",
       title="Yearly Profit Distribution From Almond Yield Parameter Uncertainty",
       caption="Note: 1995 has been separated due to large difference in scale.")
p1

ggsave("profit_sensitivity.jpg", p1)
```

The same plot, with 1995 included, is below.

```{r}
p2 <- profit_sensitivity %>% ggplot(aes(x=factor(year), y=profit, group=year)) +
  geom_boxplot() +
  theme_light() +
  theme(axis.text.x = element_text(angle=90)) +
  labs(x="Year", y="Profit ($/acre)",
