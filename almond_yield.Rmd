...
* Assume +-15% uncertainty in *temp_param* and *precip_param* parameters, using a uniform distribution

* Default values from the function are -0.015 and -0.07, respectively
...
temp_param = rnorm(mean = base_temp, sd = deviation, n = nsamples)
precip_param = rnorm(mean = base_precip, sd = deviation, n = nsamples)

# Combine into data frame
params = cbind.data.frame(temp_param, precip_param)

# Run yield model on all parameter combinations
results = params %>% pmap(yield_model, 
                          clim=clim)

# Extract results
mean_yield = map_df(results, `[`, c("almond_yield"))
mean_yield = cbind.data.frame(mean_yield, params)

# Plot results
p1 = ggplot(mean_yield, 
            aes(temp_param, almond_yield, col=precip_param)) +
  geom_point(cex=2) +
  labs(y="Mean Annual Almond_Yield", 
       x="Temperature Coefficient")

p2 = ggplot(mean_yield, 
            aes(precip_param, almond_yield, col=temp_param)) + 
  geom_point(cex=2) +
